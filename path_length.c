...
void new_pl_calc(){
	...
	//TH1F *Theta = new TH1F("Theta",";K+ CM Angle;counts",200,-2.0,2.0);
	float x[n], y[n];
...
float CthCM(float egam, float p){
	...
	//float const egam=1.22, plam[2]={1.0,1.1};
	...
	// Loop from low to high of momentum bin
	/*
	for(i=0; i<2; i++) {
		elamlab = sqrt(p[i]*p[i] + mlam*mlam);
		...
		costhcmKp[i] = (tmandel - mkp*mkp + 2.0*egamcm*ekpcm)/(2.0*egamcm*pkpcm);
		cout << "pLambda = " << p[i];
		cout << "  CosThCM_K+ = " << costhcmKp[i] << endl;
	}
	*/
	...
	//cout << "pLambda = " << p;
	//cout << "  CosThCM_K+ = " << costhcmKp << endl;

	// Output the final result
	//cout << "Egamma = " << egam << "  Delta(costh) = " << delta << endl;
	
	//if (costhcmKp<-1.0){ costhcmKp=-1.0; }
	//if (costhcmKp>1.0){ costhcmKp=1.0; }
...
float CthL(float egam, float p){	
	...
	float pL = sqrt(Elam*Elam-mlam*mlam);
	float cthcm = CthCM(egam, p);
	float cthL = -cos( asin((pcm/pL)*sin(acos(cthcm))) );
	//float cthL = cos( asin((pcm/pL)*sin(acos(cthcm))) );
